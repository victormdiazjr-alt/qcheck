...
/* ------------------------------------------------------------ la puerta de escritura
   ...
   Gemela de la del worker (Q-142). Los dos servidores comparten el TEXTO, no
   las funciones — asi que si se toca una, se toca la otra. */
...
function crearAlmacen(archivo) {
  ...
  return {
    ...
    /* EL ESTADO, NO LA HISTORIA — Q-141, 29 de agosto de 2026.

       Gemelo de `/api/estado` en el worker; el porqué está escrito allí. De
       todas las líneas del registro solo gana una por cada (ent, id, campo):
       las demás son versiones viejas de ese mismo dato. Aplicar solo las
       ganadoras, en orden, da exactamente el mismo estado — porque aplicar una
       línea es siempre «este campo ahora vale esto».

       Un aparato que se estrena pide esto y queda al día de un viaje, en vez de
       reproducir la historia entera de 2.000 en 2.000. */
    /* Q-150: `dias` recorta los ENSAYOS a la ventana que el aparato va a
       llevar. Las obras, los limites y las fichas de dia van siempre: son pocas
       y son lo que hace falta para juzgar — y juzgar sin limites fue justo lo
       que paso la mañana del 29 de agosto. */
    estado(dias) {
      const gana = new Map();
      for (const o of ops) gana.set(o.ent + "|" + o.id + "|" + o.campo, o);
      const todas = [...gana.values()].sort((a, b) => a.seq - b.seq);
      if (!dias) return todas;
      const corte = new Date(Date.now() - dias * 86400000).toISOString().slice(0, 10);
      const vivos = new Set();
      for (const o of todas) {
        if (o.ent === "test" && o.campo === "date" && String(o.valor) >= corte) vivos.add(String(o.id));
      }
      return todas.filter((o) => o.ent !== "test" || vivos.has(String(o.id)));
    },

    /* ¿ESTA YA ESTA AQUI? — Q-147, 29 de agosto de 2026.

       El gemelo de Cloudflare contesta `aceptadas` preguntandole a la tabla
       («dame los uid que tengo de esta lista»), y este contestaba `nuevas`
       («las que acabo de meter»). Parece lo mismo y no lo es: **la segunda vez
       que llega la misma linea, la respuesta es vacia**.

       Y la misma linea llega dos veces continuamente, porque para eso existe la
       cola: el aparato empuja, se cae la señal antes de que vuelva la
       respuesta, y al recuperarla empuja lo mismo otra vez. El servidor la
       tenia guardada desde el primer intento, contestaba «no he aceptado nada»,
       y el aparato no la descolgaba. Reenviandola cada tres segundos, para
       siempre, con la franja diciendo que todo va bien.

       Lo encontro el ensayo general: 13 apuntes enviados, 0 aceptadas, 0
       rechazadas, cola de 13 que no bajaba de 13.

       > Confirmar no es «la he metido yo ahora». Es «la tengo».

       Que es justo lo que hace falta que signifique cuando lo que hay entre los
       dos es una raya de cobertura. */
    conoce(uid) { return vistos.has(String(uid)); },
...
function montarAPI(almacen, token, opciones) {
  ...
  async function atenderAsync(req, res, url) {
    ...
    if (url.pathname === "/api/estado" && req.method === "GET") {
      if (exige && !quien) return responder(res, 401, { error: "sesion" });
      /* Solo ent/id/campo/valor: lo demas es auditoria y vive en el registro.
         Y `dias` recorta a la ventana del aparato (Q-150). */
      const dias = Number(url.searchParams.get("dias") || 0) || 0;
      const est = almacen.estado(dias).map((o) => ({ ent: o.ent, id: o.id, campo: o.campo, valor: o.valor }));
      return responder(res, 200, { seq: almacen.seq(), estado: true, ventana: dias || null, ops: est });
    }
    ...
    if (url.pathname === "/api/cambios" && req.method === "POST") {
      ...
      /* Q-142 — la puerta de escritura, gemela de la del worker. El porqué está
         escrito allí: el 29 de agosto un aparato subió un plan vacío y borró
         los límites de la obra para todos, en pleno tiro. */
      const rechazadas = [];
      const pasan = [];
      for (const o of (Array.isArray(datos.ops) ? datos.ops : [])) {
        const motivo = revisarOp(o);
        if (motivo) { rechazadas.push({ uid: String((o && o.uid) || ""), ent: (o && o.ent) || "", campo: (o && o.campo) || "", motivo }); continue; }
        pasan.push(o);
      }
      if (rechazadas.length) {
        console.warn(`QCheck: ${rechazadas.length} línea(s) rechazadas en la puerta — ` +
          rechazadas.slice(0, 8).map((r) => `${r.ent}.${r.campo}: ${r.motivo}`).join(" | "));
      }
      almacen.anadir(pasan, quien ? quien.usr : null);
      /* Q-147: se confirma lo que el servidor TIENE, no lo que acaba de meter.
         Igual que el gemelo de Cloudflare, que pregunta `SELECT uid FROM ops
         WHERE uid IN (...)`. Un reintento tiene que poder descolgarse. */
      const aceptadas = pasan.map((o) => String((o && o.uid) || "")).filter((u) => almacen.conoce(u));
      return responder(res, 200, { seq: almacen.seq(), aceptadas, rechazadas });
    }
